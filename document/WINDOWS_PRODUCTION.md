...
本文约定 R18 设计的 Windows 生产运行方式。仓库已经提供 Waitress、WinSW、Nginx 兼容配置和自动化契约，但不要求当前直接运行 Git clone 的线上站点在首次安全升级前先迁移到这套目录和服务体系。当前 R20 使用 `R20_GIT_DEPLOYMENT.md` 的精简流程，先识别真实启动方式，再决定是否把 WinSW 作为后续独立改进。
