...
当前导出格式为 v5。新版本导入器同时接受历史 v1、v2、v3、v4 和当前 v5：

- v5 与 v4 使用相同字段投影，但冻结了新增的 `moderator_takedown` 限制状态和同名审核日志动作；管理员主动下架的原因、时间、操作人和通知必须完整保存，不得降级为举报下架或改写审核状态。
...
- 比较器不连接数据库，也不信任 manifest 中的摘要声明；它重新读取并校验冻结的 v3、v4 或 v5 目录、canonical JSONL 字节、实际 SHA、记录数量、模型和主键序列；两端格式版本必须一致。
...
- 目标库已有不同数据时拒绝导入；v3–v5 同时比较结构投影、依赖、migration、实体摘要和序列下限，v2 使用冻结字段摘要，v1 额外校验推导后的限制语义。
...
- `shares/0029` 只增加分享与合集的回收站元数据、索引和一致性约束，并为恢复动作扩展日志枚举；所有历史记录保持未删除默认值，不删除或改写既有内容及关系。SQLite 正向与反向重建表前会分别捕获分享和合集的 AUTOINCREMENT 高水位，完成后只升不降地恢复。回收站存在任何内容时，反向迁移会在删除字段前拒绝执行；生产回滚必须恢复完整备份。
- `shares/0030` 只扩展已有内容限制与审核日志枚举，并更新限制状态检查约束，不增加或改写任何分享行；SQLite 正向与反向重建分享表时同样保护 AUTOINCREMENT 高水位。管理员下架状态或对应审计日志一旦产生，反向迁移会拒绝让这些语义变成旧版本无法识别的数据；生产回滚必须恢复完整备份。
...
- 升级源库与最终目标的 `sqlite_schema` 和 `table_structures` 必须精确相等；升级源库保留全部原始序列下限，最终目标按上述 v3–v5 范围检查 `max(original source, upgraded source)`。被排除的内嵌桥接、框架元数据、会话及其它序列必须逐项附原因。
