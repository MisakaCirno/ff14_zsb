...
- 本地测试数据库仍停留在 `shares/0025`，本次开发没有就地修改它；统一启动器会在需要时提示是否对备份候选库执行 `0026`–`0030`，拒绝升级则保持停止。
- Fast/Full/Release 分档，所有步骤输出独立耗时；Django 测试支持 4 worker 并行。当前 Fast 为 89 秒。
- 移除未使用的 Alpine 运行时，生产 JS 从约 168.30 KB 降至 111.59 KB，gzip 从约 49.26 KB 降至 32.28 KB。
- 将数据可移植模块拆分为 schema、codec、projection、I/O 和事务编排层；当前 v5 在 v4 回收站契约上加入管理员下架语义，冻结的 v1–v4 数据格式保持兼容。
- 生产数据库升级新增独立只读语义门禁：源表、源列、已有行和序列下限必须保留，新增行与默认值必须命中精确白名单；`0029`/`0030` 对 SQLite 表重建显式保存并恢复分享及合集的自增高水位，已用线上 `0018` 不可变快照演练至 `0030`，并以高水位极端夹具完成两轮端到端验证。
- 分享与合集删除改为可恢复回收站；点赞、收藏、举报、合集成员和审计日志不再因网页删除而级联丢失，用户与分享的 Django Admin 物理删除入口已关闭。
- 每条分享继续保留非空审核状态；上线后的管理员主动下架使用独立限制状态，不改写原审核结论或可见性。管理员必须填写说明，系统同时保存限制元数据、审计日志和作者站内信，并可通过现有复核流程解除。
...
- Fast：625 项 Django 测试通过，7 项按环境跳过；78 项 Vitest 通过；设计系统、77 组颜色对比度、TypeScript、ESLint、Vite、Windows 运维、WinSW 和 Nginx 契约通过。
