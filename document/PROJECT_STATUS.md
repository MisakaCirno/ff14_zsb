...
- Fast：583 项 Django 测试通过，5 项按环境跳过；78 项 Vitest 通过；设计系统、77 组颜色对比度、TypeScript、ESLint、Vite、Windows 运维、WinSW 和 Nginx 契约通过。
...
1. 只读审查真实 Nginx、TLS、域名、`/n/` 和 Windows 服务拓扑。
2. 准备 R20 go/no-go 运行单、维护窗口、停写证明、最终备份、管理员密码轮换和回滚演练。
3. 冻结发布版本并在隔离路径验证 Windows 服务、Nginx 恢复包、切换和回滚步骤。
4. 获得明确上线授权后才执行正式迁移与切换。
