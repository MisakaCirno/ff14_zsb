...
同一瀑布流 sentinel 通过 `hx-sync="this:drop"` 约束并发：已有 continuation 请求时，新的视口交叉或按钮触发请求直接丢弃，不在失败后排队重放。键盘明确触发的请求完成后，浏览器将焦点移入本次新增卡片；没有新卡片时依次回退到下一页 sentinel 或结束状态。请求失败时原 sentinel 恢复为可重试按钮并保留键盘焦点。
...
浏览器端失败反馈使用固定文案，不读取或回显响应正文。点赞或收藏 POST 失败时保留原按钮状态；POST 已成功但“我的点赞／收藏”权威 section 刷新失败时，明确提示用户刷新页面，并清理待恢复焦点，不能把陈旧列表误报为已同步。单次互动请求在 `htmx:beforeSend` 按请求标识绑定直接生命周期监听，因此按钮先被 section 的 `outerHTML` 更新移出 DOM 后，失败反馈和请求状态清理仍然有效；文档级委托继续处理仍连接的元素和 section 刷新。
...
审核拒绝、限制确认／解除和举报处理端点的成功 POST 继续使用安全站内重定向；服务端表单校验失败时不得通过消息后重定向丢弃输入，而是返回状态码 `400` 的完整审核或举报页面：

- 保留原处理说明、服务端确定的分享／举报对象和操作 action，不接受客户端伪造的目标上下文。
- `return_page` 只作为正整数页码交给 Paginator 解析；外站 URL、无效页码和越界页码不能形成重定向或错误分页链接。
- 错误页使用对应管理列表的 GET 地址生成分页，保持审核或举报页签的 `active` 与 `aria-current="page"`。
- 目标已经离开队列时仍可渲染单个恢复上下文，但不得执行业务写入；举报错误页只展开目标分享。
- 所有 POST 表单必须携带 CSRF 字段，包括经过带 `only` 的嵌套模板 include 渲染的审核模态框。

浏览器收到错误页后自动打开唯一的恢复模态框并聚焦错误摘要；关闭时把焦点还给匹配 action 的触发按钮，触发按钮不存在时回退到 `#main-content`。后续普通打开共享举报模态框时必须清除上一轮错误、陈旧提示、说明、ARIA 错误状态和 action，不能把旧操作上下文带入新请求。
