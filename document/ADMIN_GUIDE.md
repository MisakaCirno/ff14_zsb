...
#### 批量操作
- ✅ 批量设为公开（仅处理已审核通过且没有内容限制的分享）
- ✅ 批量设为私有（可收紧任意分享的可见性）

批量公开不会代替审核，也不会解除审核拒绝、举报下架或历史限制；不符合条件的项目会被跳过，并在结果消息中分别显示已更新、无需修改和被阻止的数量。批量设为私有同样不会改写审核状态或限制证据。两项操作都只为实际变化写入分享日志和 Django 管理日志，重复执行不会产生重复日志。

#### 详细编辑
分组显示：
- **基本信息**：标题、作者、分享ID、可见性、审核状态
- **内容**：战术板代码、描述
