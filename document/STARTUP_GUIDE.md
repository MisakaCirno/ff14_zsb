...
项目不提供默认管理员凭据。首次访问管理后台前运行：

```bash
python manage.py createsuperuser
```

按提示交互输入用户名和强密码，然后访问 http://127.0.0.1:8000/admin/ 。不要把密码写入脚本、文档或版本库。需要重置时运行 `python manage.py changepassword <管理员用户名>`。
...
- 在详情页可以看到：
  ...
  - 战术板预览（图片）
  - 带二维码的分享图生成与下载
- 点击“生成分享图”可获得包含预览、标题、作者和详情链接二维码的图片
